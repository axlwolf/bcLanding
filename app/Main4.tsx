...
import {
  ...
  ServiceSection as ServicesSectionType, // Corrected typo: ServicesSection -> ServiceSection
...
async function getLandingContentData(slug: string): Promise<ProductSaaSLandingContent | null> {
  try {
    ...
    if (!res.ok) {
      console.error(
        `Failed to fetch landing content (Main4) for slug ${slug}: ${res.status} ${res.statusText}`
      )
      return null
    }
    ...
    if (data.pageType !== 'product' && data.pageType !== 'saas') {
      console.warn(
        `Fetched content for Main4 (slug ${slug}) is not of type product/saas. PageType: ${data.pageType}`
      )
    }
...
const MainFeaturesSectionClient = ({
  mainFeatures,
}: {
  mainFeatures: MainFeaturesSectionType | undefined
}) => {
  'use client'
  const ref = useRef(null)
  const isInView = useInView(ref, { once: false, amount: 0.2 })
      ...
      >
        ...
        <motion.div className="grid grid-cols-1 gap-12 lg:grid-cols-3" variants={staggerContainer}>
          {mainFeatures.items.map((feature, index) => {
            const IconComponent =
              featureIconMap[feature.icon as keyof typeof featureIconMap] || FaBolt
            return (
              <motion.div
                key={index}
      ...
      >
        ...
        >
          {features.items.map((feature, index) => {
            const IconComponent =
              featureIconMap[feature.icon as keyof typeof featureIconMap] || FiSettings
            return (
              <motion.div
                key={index}
...
const TestimonialsSectionClient = ({
  testimonials,
}: {
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          {cta.collectEmail ? (
            <form
              ...
              onSubmit={async (e) => {
                e.preventDefault()
                await subscribe(email) /* if (status === 'success') setEmail(''); */
              }}
            >
                    ...
                    <div className="relative z-10">
                      <div className="mb-4 flex items-center">
                        ...
                        <h3 className="text-2xl font-bold">
                          {contact.emergencyService?.title}
                        </h3>{' '}
                      </div>
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-10 text-center sm:px-6 xl:max-w-5xl xl:px-0">
        <p className="text-lg text-red-500">
          Failed to load content for Main4 page. Please try again later.
        </p>
      </div>
    )
  }

  const { hero, services, projects, mainFeatures, features, testimonials, cta, faqs, contact } =
    landingContent
