'use client' // Main3 is a Client Component

import React, { useState, useEffect } from 'react' // Keep for client components
...
import ImportedGallerySection from '@/components/GallerySection' // Ensure this is client-compatible or refactored

import { useEmailSubscription } from '@/lib/useEmailSubscription'
import FeatureIcon from '@/components/FeatureIcon'
import { HiCheckCircle } from 'react-icons/hi2'
import Main3ContactSection from './Main3ContactSection' // Assuming this is client component or takes props correctly
...
async function fetchLandingContentClient(slug: string): Promise<ProductSaaSLandingContent | null> {
  try {
    ...
    if (typeof data !== 'object' || data === null) {
      console.error(`Fetched data is not an object for slug ${slug} (Main3). Received:`, data);
      return null;
    }
    if (data.pageType !== 'product' && data.pageType !== 'saas') {
        console.warn(`Fetched content for Main3 (slug ${slug}) is of pageType '${data.pageType}'.`)
    }
...
// Define sub-components. If they use hooks, they are fine as Main3 is 'use client'.
...
const imageMap = {
  landingDemoImage: landingDemoImage.src,
...
const FeaturesSectionClient = ({ mainFeatures }: { mainFeatures: MainFeaturesSectionType | undefined }) => {
  // This sub-component uses useState, so it's fine within Main3 (Client Component)
  ...
  useEffect(() => {
    if (mainFeatures?.items?.length > 0) {
      ...
      if (!currentSelectionExists || !selectedFeature) {
        setSelectedFeature(mainFeatures.items[0]);
      }
    } else if (mainFeatures?.items?.length === 0) {
              ...
              <div className="mt-10 w-[45rem] overflow-hidden rounded-xl bg-slate-50 shadow-xl shadow-blue-900/20 sm:w-auto lg:mt-0 lg:w-[67.8125rem]">
                {selectedFeature?.image && (
                  <Image
                    ...
                    src={imageMap[selectedFeature.image as keyof typeof imageMap] || selectedFeature.image}
                  />
                )}
              </div>
            </div>
          ...
          {features.items.map((feature) => (
            <div key={feature.title}>
              <div className="mx-auto max-w-2xl">
                <div className="w-9"><FeatureIcon icon={feature.icon} /></div>
                <h3 className="text-primary-600 dark:text-primary-200 mt-6 text-xl font-medium">{feature.title}</h3>
                <p className="font-display mt-2 text-lg text-slate-900 dark:text-slate-200">{feature.description}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="hidden lg:mt-20 lg:block">
          <div className="mx-auto grid max-w-7xl grid-cols-3 gap-x-8 px-4 sm:px-6 lg:px-8" role="tablist" aria-orientation="horizontal" >
            {features.items.map((feature) => (
              <div key={feature.title}>
                <div className="relative my-6 opacity-90 hover:opacity-100">
                  <div className="w-9"><FeatureIcon icon={feature.icon} /></div>
                  <h3 className="text-primary-600 dark:text-primary-200 mt-3 text-xl font-medium">{feature.title}</h3>
                  <p className="mt-2 text-lg text-slate-900 dark:text-slate-200">{feature.description}</p>
                </div>
              </div>
            ))}
          </div>
...
const CallToActionSectionClient = ({ cta }: { cta: CtaSectionType | undefined }) => {
    ...
    <section id="cta" className="relative overflow-hidden bg-gray-900 py-32 dark:bg-gray-500">
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-lg text-center">
          <h2 className="font-display text-3xl tracking-tight text-white sm:text-4xl dark:text-slate-900"> {cta.title} </h2>
          <p className="mt-4 text-lg tracking-tight text-white dark:text-slate-900"> {cta.description} </p>
          <div className="mt-8 flex justify-center">
            {cta.collectEmail ? (
              <form className="flex flex-col items-center justify-center gap-4" onSubmit={async (e) => { e.preventDefault(); await subscribe(email); /* if (status === 'success') setEmail('') */ }} >
                <div className="flex flex-col items-center justify-center gap-4 sm:flex-row sm:justify-center">
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email" className="focus:border-primary-500 focus:ring-primary-200 focus:ring-opacity-50 rounded-md border border-gray-300 bg-slate-50 px-4 py-2 text-base focus:ring dark:border-gray-600 dark:text-gray-800" disabled={status === 'loading'} />
                  <button type="submit" className="bg-primary-500 hover:bg-primary-600 rounded-md px-8 py-2 text-base font-medium text-white disabled:opacity-60" disabled={status === 'loading'} >
                    {status === 'loading' ? 'Submitting...' : cta.button.text}
                  </button>
                </div>
                <div className="mt-2 flex h-6 w-full items-center justify-center"> {message && ( <span className="text-primary-600 dark:text-primary-400 text-sm"> {message} </span> )} </div>
              </form>
            ) : (
              <Link href={cta.button.link} className="bg-primary-500 hover:bg-primary-600 inline-flex items-center justify-center rounded-md border border-transparent px-6 py-3 text-base font-medium text-white md:px-8 md:py-4 md:text-lg" >
                {cta.button.text}
              </Link>
            )}
          </div>
...
const TestimonialsSection = ({ testimonials }: { testimonials: TestimonialSectionType | undefined }) => {
  if (!testimonials?.testimonials || testimonials.testimonials.length === 0) return null;
  return (
    <section id="testimonials" aria-label="What our customers are saying" className="bg-slate-50 py-20 sm:py-32 dark:bg-gray-900" >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl md:text-center">
          <h2 className="font-display text-3xl tracking-tight text-slate-900 sm:text-4xl dark:text-white"> {testimonials.title} </h2>
          <p className="mt-4 text-lg tracking-tight text-slate-700 dark:text-white"> {testimonials.description} </p>
        </div>
              ...
              <ul className="flex flex-col gap-y-6 sm:gap-y-8">
                <li>
                  <figure className="relative rounded-2xl bg-white p-6 shadow-xl shadow-slate-900/10 dark:bg-gray-800">
                    <blockquote className="relative"> <p className="text-lg tracking-tight text-slate-900 dark:text-white"> {testimonial.quote} </p> </blockquote>
                    <figcaption className="relative mt-6 flex items-center justify-between border-t border-slate-100 pt-6">
                      <div>
                        <div className="font-display text-base text-slate-900 dark:text-white"> {testimonial.name} </div>
                        <div className="mt-1 text-sm text-slate-500 dark:text-slate-400"> {testimonial.title} </div>
                      </div>
                      <div className="overflow-hidden rounded-full bg-slate-50 dark:bg-gray-800">
                        <Image alt={testimonial.name || ""} loading="lazy" width="56" height="56" decoding="async" data-nimg="1" className="h-14 w-14 object-cover" style={{ color: 'transparent' }} src={testimonial.image} />
                      </div>
                    </figcaption>
                  </figure>
                </li>
...
const FaqSection = ({ faqs }: { faqs: FaqsSectionType | undefined }) => {
  ...
  return (
    <section id="faq" aria-labelledby="faq-title" className="relative overflow-hidden bg-slate-50 py-20 sm:py-32 dark:bg-gray-800" >
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl lg:mx-0">
          <h2 id="faq-title" className="font-display text-3xl tracking-tight text-slate-900 sm:text-4xl dark:text-white" > {faqs.title} </h2>
          <p className="mt-4 text-lg tracking-tight text-slate-700 dark:text-slate-200">
          ...
          {faqs.questions.map((faq, index) => (
            <li key={index}>
              <ul className="flex flex-col gap-y-8">
                <li>
                  <h3 className="font-display text-lg/7 text-slate-900 dark:text-white"> {faq.question} </h3>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="md:text-center">
          <h2 className="font-display text-3xl tracking-tight text-white sm:text-4xl">
            <span className="relative whitespace-nowrap">
              <svg aria-hidden="true" viewBox="0 0 281 40" preserveAspectRatio="none" className="fill-primary-400 absolute top-1/2 left-0 h-[1em] w-full" >
        ...
        <div className="-mx-4 mt-16 grid max-w-2xl grid-cols-1 gap-y-10 sm:mx-auto lg:-mx-8 lg:max-w-none lg:grid-cols-3 xl:mx-0 xl:gap-x-8">
          {pricing.plans.map((plan, index) => (
            <section key={index} className={`flex flex-col rounded-3xl px-6 sm:px-8 lg:py-8 ${plan.highlighted ? 'bg-primary-600' : 'bg-slate-900'}`} >
              <h3 className="font-display mt-5 text-lg text-white">{plan.name}</h3>
              <p className={`mt-2 text-base ${plan.highlighted ? 'text-slate-100' : 'text-slate-400'}`} > {plan.description} </p>
              <p className={`font-display order-first text-5xl font-light tracking-tight text-white ${plan.highlighted ? 'text-slate-400' : 'text-slate-400'}`} > {plan.price} </p>
              <ul className={`order-last mt-10 flex flex-col gap-y-3 text-sm ${plan.highlighted ? 'text-slate-100' : 'text-slate-200'}`} >
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex">
                    <HiCheckCircle className={`h-5 w-5 flex-none ${plan.highlighted ? 'text-slate-100' : 'text-slate-400'}`} aria-hidden="true" />
...
export default function Main3({ posts }: { posts: any[] }) { // Accept posts prop
  ...
  useEffect(() => {
    async function loadData() {
      setIsLoading(true);
      const data = await fetchLandingContentClient('main-landing'); // or a specific slug for Main3
      if (data) {
        setLandingContent(data);
      } else {
        setError("Failed to load page content for Main3.");
      }
      setIsLoading(false);
    }
    loadData();
  ...
  if (isLoading) {
    return <div className="flex h-screen items-center justify-center"><p>Loading Main3...</p></div>;
  }
