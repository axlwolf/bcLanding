...
async function migrate() {
  ...
  try {
    ...
    if (fetchError && fetchError.code !== 'PGRST116') {
      // PGRST116: 'Fetched row returned no data' (expected if page doesn't exist)
    ...
    if (existingPage) {
      console.log(
        `Found existing page with ID: ${existingPage.id}. Deleting its content and the page itself...`
      )
    ...
    console.log(
      `Inserting into landing_pages: slug='${DEFAULT_LANDING_PAGE_SLUG}', page_type='${pageType}', name='${DEFAULT_LANDING_PAGE_NAME}'`
    )
